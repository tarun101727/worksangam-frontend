...
const PaymentHistory = () => {
  ...
  // Map raw backend statuses to translated labels
  const statusMap = {
    SUCCESS: t("SUCCESS"),
    FAILED: t("FAILED"),
    PENDING: t("PENDING"),
  ...
  const fetchPayments = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${BASE_URL}/api/payment/my-payments`, {
  ...
  const totalCredits = successPayments.reduce((acc, p) => acc + p.credits, 0);
    ...
    <div className="min-h-screen px-4 py-10 text-white">
      ...
      <div className="grid md:grid-cols-4 gap-4 max-w-5xl mx-auto mb-6">
        ...
        <div className="bg-white/5 p-4 rounded-xl border border-white/10">
          ...
          <h3 className="text-xl font-bold text-green-400">{successCount}</h3>
        </div>

        {/* FAILED */}
        <div className="bg-white/5 p-4 rounded-xl border border-white/10">
          <p className="text-white/60 text-sm">{t("Failed Payments")}</p>
          <h3 className="text-xl font-bold text-red-400">{failedCount}</h3>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center text-white/60 mt-20">
          ...
          <p className="text-sm">{t("Your transactions will appear here")}</p>
        </div>
      ) : (
        // 💳 LIST
        <div className="max-w-4xl mx-auto space-y-4">
